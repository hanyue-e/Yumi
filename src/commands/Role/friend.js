...
module.exports = {
  ...
  category: "Role",
  aliases: ["mittar", "fri"],
  description: "Assigns the 'friend' role to a specified user.",
  args: true,
  usage: "<user>",
  userPerms: [],
  owner: false,
  execute: async (message, args, client, prefix) => {
    try {
      ...
      // Check for the friend role
      const friendRole = guildRoles.friend; // Assuming friendRole is a single role ID
      if (!friendRole) {
        return message.channel.send(v2({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `There is no **friend Role** set for **Custom Roles**.`,
              ),
          ],
        }));
      }

      // Get the role object for the friend role
      const roleToAssign = message.guild.roles.cache.get(friendRole);
      if (!roleToAssign) {
        return message.channel.send(v2({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `I couldn't find the **friend role** in this server. It might have been deleted!`,
              ),
          ],
        }));
      }
