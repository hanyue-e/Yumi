...
module.exports = {
  ...
  category: "Role",
  aliases: ["qt"],
  description: "Assigns the 'girl' role to a specified user.",
  args: true,
  usage: "<user>",
  userPerms: [],
  owner: false,
  execute: async (message, args, client, prefix) => {
    try {
      ...
      // Check for the girl role
      const girlRole = guildRoles.girl; // Assuming girlRole is a single role ID
      if (!girlRole) {
        return message.channel.send(v2({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `There is no **girl Role** set for **Custom Roles**.`,
              ),
          ],
        }));
      }

      // Get the role object for the girl role
      const roleToAssign = message.guild.roles.cache.get(girlRole);
      if (!roleToAssign) {
        return message.channel.send(v2({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `I couldn't find the **girl role** in this server. It might have been deleted!`,
              ),
          ],
        }));
      }
