...
module.exports = {
  ...
  category: "Role",
  aliases: ["staff"],
  description: "Assigns the 'official' role to a specified user.",
  args: true,
  usage: "<user>",
  userPerms: [],
  owner: false,
  execute: async (message, args, client, prefix) => {
    try {
      ...
      // Check for the official role
      const officialRole = guildRoles.official; // Assuming officialRole is a single role ID
      if (!officialRole) {
        return message.channel.send(v2({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `There is no **official Role** set for **Custom Roles**.`,
              ),
          ],
        }));
      }

      // Get the role object for the official role
      const roleToAssign = message.guild.roles.cache.get(officialRole);
      if (!roleToAssign) {
        return message.channel.send(v2({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `I couldn't find the **official role** in this server. It might have been deleted!`,
              ),
          ],
        }));
      }
