...
const state = {
  me: null,
  guilds: [],
  guildId: localStorage.getItem("arrkii:guild") || "",
  data: null,
  draft: null,
  page: localStorage.getItem("arrkii:page") || "overview",
  welcomeTab: localStorage.getItem("arrkii:welcomeTab") || "messages",
  dynamicImageIndex: Number(localStorage.getItem("arrkii:dynamicImageIndex") || 0),
  selectedLayer: localStorage.getItem("arrkii:selectedLayer") || "avatar",
  dirty: false,
  saving: false,
  refreshing: false,
};
...
async function init() {
  try {
    ...
    if (forceServerChooser) {
      ...
      state.guildId = "";
      localStorage.removeItem("arrkii:guild");
    }
        ...
        <section class="page wide server-chooser-page">
          <div class="server-hero">
            <div class="page-title">
              ...
              <div>
                <h1>Select Server</h1>
                <p>Pick a server to manage, or invite Arrkii where it is missing.</p>
              </div>
            </div>
  ...
  return `
    <article class="server-card ${status}" data-server-card data-server-status="${status}" data-server-name="${escapeAttr(guildName.toLowerCase())}">
      ...
      <div class="server-card-main">
        ...
        <p>${guild.installed ? "Dashboard is ready for this server." : "Invite Arrkii before managing this server."}</p>
      </div>
...
async function loadGuild(guildId) {
  ...
  localStorage.setItem("arrkii:guild", guildId);
  state.dirty = false;
  state.data = null;
  state.draft = null;
  renderShell(true);
...
function renderOverview() {
  ...
  return `
    <section class="page wide">
      ...
      <div class="grid">
        ${shortcutCard("Custom prefix", "General Settings", "Update how members call Arrkii commands.", "general", "Open settings")}
...
function bindGlobalActions() {
  ...
  app.querySelectorAll("[data-welcome-tab]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      localStorage.setItem("arrkii:welcomeTab", state.welcomeTab);
      renderShell();
    });
  });
  app.querySelectorAll("[data-dynamic-select]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      localStorage.setItem("arrkii:dynamicImageIndex", String(state.dynamicImageIndex));
      renderShell();
  ...
  app.querySelectorAll("[data-layer-select]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      localStorage.setItem("arrkii:selectedLayer", state.selectedLayer);
      renderShell();
    });
  });
...
function bindSidebar() {
  app.querySelectorAll("[data-nav]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      localStorage.setItem("arrkii:page", state.page);
      renderShell();
    });
  });
}
...
function bindDynamicEditorDrag() {
  ...
  svg.addEventListener("pointerdown", (event) => {
    ...
    localStorage.setItem("arrkii:selectedLayer", layerId);
